...
foreach ($data as $v) {
    ...
    $cancelQty += $v['cancel_qty'];
            ...
            <tr>
                ...
                <th>Order No.</th>
                <th>Date</th>
                <th style="max-width: 300px;">Customer</th>
                <th style="max-width: 100px;">City</th>
                <th>O QTY<br><b><?= $qtyTotal; ?></b></th>
                <th>D QTY<br><b><?= $dispatchQty; ?></b></th>
                <th>P QTY <br><b><?= $pendingQty; ?></b></th>
                <th>C QTY <br><b><?= $cancelQty; ?></b></th>
...
foreach ($data as $v):
    ...
    $pendingQty += $v['pending_qty'];
    $cancelQty += $v['cancel_qty'];
    $onlineUser = $v['status'] == 'Pending' ? 'background-color: #efa1a1;' : '';
            ?>
            <tr style="{{ $onlineUser }}">
    ...
    if ($editPermission && $v['cancel_qty'] == 0) {?>
                        <a href="{{route('order.edit', ['order' => $v['id']])}}" data-bs-toggle='tooltip'
                            ...
                            class="btn btn-action bg-warning text-white me-1"><i class="fas fa-edit"></i></a>
                        <?php    }
    ...
    if ($editPermission) {?>
                        <a href="{{route('order.delete', ['order' => $v['id']])}}" data-bs-toggle='tooltip'
                            ...
                            class="btn btn-action bg-danger text-white me-1 btn-delete"><i class="fas fa-trash"></i></a>
                        <?php    } ?>
                        <a href="{{route('order.view', ['order' => $v['id']])}}" data-bs-toggle='tooltip'
                            data-bs-placement='top' data-bs-original-title='View'
                            class="btn btn-action bg-primary text-white me-1"><i class="fas fa-eye"></i></a>
    ...
    if ($otherPermission && $v['cancel_qty'] == 0) {?>
                        <a href="{{route('order.cancel', ['order' => $v['id']])}}" data-bs-toggle='tooltip'
                            ...
                            class="btn btn-action bg-danger confirmClick text-white me-1"><i
    ...
    if ($otherPermission && $v['cancel_qty'] > 0) {?>
                ...
                <!-- <td style="color: <?php    //$v['color']?>;">
                        @if ($v['order_type'] == 'online' && $v['status'] == 'Pending')
                        <select class="form-select status" data-id="{{ $v['id'] }}">
                ...
                <td style="color: <?=$v['color']?>;"><?= $v['print_type'] ?></td>
                <!-- <td style="color: <?php    //$v['color']?>;"><?php    // $v['po_no'] ?></td> -->
                ...
                <td style="color: <?=$v['color']?>; white-space:revert;width:265px !important;"><?= $v['customer'] ?> (
                    <?=$v['party_type']?> )
                </td>
                <td style="color: <?=$v['color']?>;max-width: 100px;"><?= $v['city']; ?></td>
