            ...
            <div class="card">
                ...
                <div class="card-body">
                        ...
                        <div class="col-md-12 mb-3">
                            <div class="row">
                                <div class="col-md-4 pb-1">
                                    <label class="col-form-label">Party Name : <span class="text-danger">*</span>&nbsp;  <span style="color: red;" id="partyDetail"></span></label>
                                    <select class="form-select ajax-customer @error('customer_id') is-invalid @enderror" id="customer" name="customer_id" required>
                                        <option value="{{ optional($estimate ?? null)->customer?->id }}" data-color="{{ optional($estimate ?? null)->customer?->partyType?->color }}" data-balance="{{optional($estimate ?? null)->customer?->balance}}" data-party_type_id="{{optional($estimate ?? null)->customer?->partyType?->id}}" data-group_name="{{optional($estimate ?? null)->customer?->partyGroup?->name}}" data-category_name="{{optional($estimate ?? null)->customer?->partyCategory?->name}}" selected >{{ optional($estimate ?? null)->customer?->name }}-({{optional($estimate ?? null)->customer?->city?->name}} - {{optional($estimate ?? null)->customer?->partyType?->name}})</option>
                                    </select>
                                </div>

                                <div class="col-md-2 pb-1">
                                    <label class='col-form-label'>Party Category:</label>
                                    <select class="form-control @error('bill_type') is-invalid @enderror select2" name="bill_type" id="bill_type">
                                ...
                                <div class="col-md-2 pb-1">
                                    <label class="col-form-label required">Transport : </label>
                                    <select class="form-control select2 @error('transport_id') is-invalid @enderror" id="transport_id" name="transport_id">
                                        <option value="">Select </option>
                                        @for ($i=0; $i < count($transport); $i++)
                                            <option value="{{ $transport[$i]->id }}" <?= isset($estimate->transport_id) && $estimate?->transport_id == $transport[$i]->id ? 'selected' : '' ?>>
                                            {{ $transport[$i]->name }} - {{ $transport[$i]->is_waybill ? 'YES' : 'NO' }}
                                            </option>
                                            @endfor
                                    </select>
                                </div>
                                                                    ...
                                                                    <table
                                                                        ...
                                                                        <thead>
                                                                            <tr>
                                                                                ...
                                                                                <td>Order Date</td>
                                                                                <td>Item</td>
                                                                                <td style="padding: 0px !important;">OPT</td>
                                                                                <td>QTY</td>
                                                                                <td>DQ</td>
                                                                                <td>PQ</td>
                                                                                <td>Rate</td>
                                    ...
                                    <div class="col-md-3">
                                        <div class="row col-md-12">
                                            <div class="col-md-6">
                                                ...
                                                <textarea style="height: 100px; width: 100%;" class='form-control @error("address") is-invalid @enderror' id="address" name="address" placeholder="Address" type="text" required>{{ old("address", $estimate?->address ?? "") }}</textarea>
                                            ...
                                            <div class="col-md-12 pt-1">
                                                ...
                                                <!-- <div class="col"> -->
                                                    <input class='form-control note @error("note") is-invalid @enderror' value='{{ old("note", $estimate?->note ?? "") }}' name="note" placeholder="Note" type="text">
                                        ...
                                        <table class="table card-table table-vcenter text-nowrap targetTable">
                                            <thead class="thead-light">
                                                <tr>
                                                    ...
                                                    <td style="padding: 0% !important;margin: 0% !important;">OPT</td>
                                                    <td>Quantity</td>
                                                    <td>Rate</td>
                                                    <td>Print Detail</td>
                                                    <td>Item Remarks</td>
                                                    ...
                                                    <td>Action</td>
                                                    <td style="padding: 0% !important;margin: 0% !important;">Parcel</td>
                                                </tr>
                                            </thead>
                                            <tbody class="append-here">
                                                ...
                                                <tr class="mainRow">
                                                    <input type="hidden" class="order-id" name="order_id[]" value="{{ old('order_id', $detail?->order_id ?? '') }}" />
                                                    <input type="hidden" name="estimate_id[]" value="{{ $detail?->id ?? '' }}" />
                                                    <input type="hidden" class="printOtherId" name="print_type_other_id[]" value="{{ $detail?->print_type_other_id ?? '' }}" />
                                                    <td style="padding: 2px !important;width: 20% !important;">
                                                        <select class="form-select item_id" required name="item_id[]">
                                                            <option value="">Select</option>
                                                    ...
                                                    <td style="padding: 0px !important;width: 0% !important;text-align: center" class="printTypeOther"></th>
                                                    <td style="padding: 2px !important;width: 6% !important;">
                                                        <input class='form-control no-spinners qty @error("qty") is-invalid @enderror' value='{{ old("qty", $detail?->qty ?? "") }}' name="qty[]" placeholder="Quantity" min="0" step="any" type="number" required>
                                                    </td>
                                                    <td style="padding: 2px !important;width: 6% !important;">
                                                        @can('estimate-other')
                                                        <input class='form-control rate no-spinners @error("rate") is-invalid @enderror' value='{{ old("rate", $detail?->rate ?? "") }}' name="rate[]" placeholder="Rate" min="0" step="any" type="number" readonly required>
                                                        @else
                                                    ...
                                                    <td style="padding: 2px !important">
                                                        <input class='form-control other_remark @error("other_remark") is-invalid @enderror' value='{{ old("other_remark", $detail?->other_remark ?? "") }}' name="other_remark[]" placeholder="Other Remark" type="text">
                                                    </td>
                                                    <td style="padding: 2px !important;width: 7% !important;">
                                                        <input class='form-control amount no-spinners @error("amount") is-invalid @enderror' readonly value='{{ old("amount", $detail?->amount ?? "") }}' name="amount[]" placeholder="Amount" min="0" step="any" type="number" required>
                                                    </td>
                                                    <td style="padding: 2px !important">
                                                        <input class='form-control date @error("date") is-invalid @enderror' value='{{ old("date", $detail?->date ?? date("Y-m-d")) }}' name="date[]" placeholder="Date" type="date">
                                                    </td>
                                                    <td style="padding: 2px !important;width: 5% !important;">
                                                        <input class='form-control discount @error("discount") is-invalid @enderror' value='{{ old("discount", $detail?->discount ?? "") }}' name="order_discount[]" placeholder="Discount" min="0" step="any" type="number" required>
                                                    </td>
                                                    ...
                                                    <td style="padding: 2px !important;width: 5% !important;">
                                                        <input class='form-control item_parcel @error("item_parcel") is-invalid @enderror' value='{{ old("item_parcel", $detail?->parcel ?? "") }}' name="item_parcel[]" placeholder="" min="0" step="any" type="number">
                                                    </td>
                                                </tr>
                                                @endforeach
                                                @else
                            ...
                            <div class="row pt-5">
                                <div class="row col-md-10">
                                    <!-- <div class="row col-md-6 pb-1">
                                        ...
                                        <div class="col">
                                            <input class="form-control" type="date" name="payment_date" value='{{ old("payment_date", $estimate?->payment_date ?? date("Y-m-d")) }}' placeholder="Enter Date" />
                                        </div>
                                    </div> -->
                                    ...
                                    <div class="row col-md-6">
                                        ...
                                        <div class="col">
                                            <textarea class="form-control" type="text" name="comments" placeholder="Enter Comments"> {{ old("comments", $estimate?->comments ?? "") }} </textarea>
                                        </div>
                                    </div>

                                    <div class="row col-md-6 pb-1">
                                        <label class="col-4 col-form-label required" id="total_label">Total Amount :</label>
                                        <div class="col">
                                            <input class="form-control no-spinners" id="total" type="number" min="0" step="any" name="total_amount" value='{{ old("total_amount", $estimate?->total_amount ?? "") }}' placeholder="Enter Amount" readonly/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row col-md-6 pt-4">
                                        ...
                                        <div class="col">
                                            <label class="form-check">
                                                <input class="form-check-input" {{ old("is_special", $estimate?->is_special ?? "") == 1 ? "checked" : "" }} name="is_special" type="checkbox">
                                                <span class="form-check-label" style="color: red">Is Special Order</span>
                                            </label>
                                        </div> -->
                                    </div>
                                    <!-- <div class="row col-md-6 pb-1">
                                        ...
                                        <div class="col">
                                            <input class="form-control no-spinners" type="number" min="0" step="any" name="payment_amount" value='{{ old("payment_amount", $estimate?->payment_amount ?? "") }}' placeholder="Enter Amount" />
                                        </div>
                                    </div> -->
                                    <div class="row col-md-6 pb-1">
                                        <label class="col-4 col-form-label">Other Charge :</label>
                                        <div class="col">
                                            <input class="form-control no-spinners" id="other_charge" type="number" min="0" step="any" name="other_charge" value='{{ old("other_charge", $estimate?->other_charge ?? "") }}' placeholder="Enter Other Charge" />
                                        </div>
                                    </div>
                                    <div class="row col-md-6 pt-1">
                                    </div>
                                    <div class="row col-md-6 pb-1">
                                        <label class="col-2 col-form-label required" id="discount_label">Dis. </label>
                                        <div class="col-2">
                                            <input class="form-control no-spinners" id="discount" type="number" min="0" step="any" name="discount" value='{{ old("discount", $estimate?->discount ?? "") }}' placeholder="Enter Discount" readonly/>
                                        </div>
                                        <div class="col">
                                            <input class="form-control no-spinners" min="0" step="any" id="discount_amount" type="number" name="discount_amount" value='{{ old("discount_amount", $estimate?->discount_amount ?? "0") }}' placeholder="Enter Discount" readonly/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row col-md-6">
                                        <label class="col-4 col-form-label" id="redeem_coin_label">Redeem Coin :</label>
                                        <div class="col">
                                            <input class="form-control no-spinners" id="redeem_coin" type="number" min="0" step="any" name="redeem_coin" value='{{ old("redeem_coin", $estimate?->redeem_coin ?? "") }}' placeholder="Enter Coin" readonly/>
                                        </div>
                                    </div>

                                    <div class="row col-md-6">
                                        <!-- <label class="col-4 col-form-label" style=""> Payment Verified : </label>
                                        <div class="col" style="padding: 10px 11px;">
                                            <input class="form-check-input" type="checkbox" name='is_verified' {{ old("is_verified", $estimate?->is_verified ?? "") == 1 ? "checked" : "" }} value='1' />
                                        </div> -->
                                    </div>

                                    <div class="row col-md-6 pt-1">
                                        <label class="col-4 col-form-label required" id="net_amount_label">Net Amount :</label>
                                        <div class="col">
                                            <input class="form-control no-spinners" id="net_amount" type="number" min="0" step="any" name="net_amount" value='{{ old("net_amount", $estimate?->net_amount ?? "") }}' placeholder="Enter Amount" readonly/>
                                        </div>
                                    </div>
                                    <!-- <div class="row col-md-6 pt-1">
                                        <label class="col-4 col-form-label">Other Charge :</label>
                                ...
                                <div class="row col-md-2">
                                    <div class="col-md-12 pb-1">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control no-spinners" id="cash_back_coin" type="number" min="0" step="any" name="cash_back_coin" value='{{ old("cash_back_coin", $estimate?->cash_back_coin ?? "") }}' placeholder="Enter Coin" readonly/>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label class="form-label">Offer Discount </label>
                                            <input class="form-control no-spinners" type="number" min="0" step="any" name="offer_discount" value='{{ old("offer_discount", $estimate?->offer_discount ?? "") }}' placeholder="Enter Offer Discount" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card-footer text-center">
                        ...
                        <a class=" btn btn-warning me-2" href="{{ route('estimate.index') }}">Back</a>
                    </div>
                </div>
            </div>
        </form>
            ...
            <tr>
                ...
                <td style="padding: 2px !important">
                    <select
                        ...
                        data-tags="true" name="transport_ids[]">
                ...
                <td style="padding: 2px !important;width: 5% !important;">
                    <button type="button" class="btn btn-danger form-control remove-btn"><i
...
<script>
    ...
    $(document).ready(function() {
        ...
        function comamanfinalamount() {
            ...
            if (netAmount > 0) {
                lastClosing = parseFloat($('#redeem_coin').data('redeem-coin') ?? 0) ?? 0;
                var balance = 0;
                ...
                if (data.party_type_id == 2) {
                    ...
                    console.log(redeemCoin);
                    $(".customer_coin_balance").html(balance);
                    if (balance >= redeemCoin) {
                        $('#redeem_coin').val(Math.round(redeemCoin));
                    } else {
                        $('#redeem_coin').val(balance);
                    }
        ...
        function addOrder(position) {
            ...
            if (orderId) {
                ...
                $('.qty, .rate, .discount, #discount').trigger('keyup');
                $('#remark-comment').val(remarkOtherOrder);
                disableSelect2();
            } else {
                var id = position.val();
        ...
        <?php if (isset($estimate) && $estimate->bill_generated == 'Yes') : ?>
        $('#rowToCloneContainer input').each(function() {
        ...
        // Custom function to "disable"
        function disableSelect2() {
            $('.item_id').data('disabled', true); // Custom flag
            $('.item_id').prop('disabled', false); // Keep enabled for JS
            $('.item_id').next('.select2-container').find('.select2-selection').addClass('select2-disabled');
        }

        // Example: Disable and get value
        disableSelect2();

        // Still get value
        let selectedVal = $('.item_id').val();
        $('#hiddenSelect').val(selectedVal); // Store for form
    });
</script>
@endpush
