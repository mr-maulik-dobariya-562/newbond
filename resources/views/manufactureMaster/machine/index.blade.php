                        ...
                        <table class="table table-loader table-vcenter card-table" id="bank-table">
                            <thead>
                                <tr>
                                    ...
                                    <th data-name="machine_type">Machine Type</th>
...
<form id="modal-form" action="{{ route('manufacture-master.machine.store') }}" method="POST">
    @csrf
    <div class="modal modal-blur fade" id="city-modal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"> <span class="title">Add</span> Machine</h5>
                ...
                <div class="modal-body">
                    <div class="row mb-3 align-items-end">
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select select2" name="machine_type_id">
...
<script>
    $(document).ready(function() {
        ...
        var table = window.table(
            "#bank-table",
            "{{ route('manufacture-master.machine.getList') }}",
        );
        ...
        $(".add-new-btn").click(function() {
            ...
            modal.parents("form").attr("action", '{{ route("manufacture-master.machine.store") }}');
            window.edit = false;
        });
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            const edit_url = "{{ route('manufacture-master.machine.update', ':id') }}";
            modal.parents("form").attr("action", edit_url.replace(":id", id));
            modal.find(".title").text("Edit");
            modal.find("input[name='name']").val(name);
            modal.find("select[name='machine_type_id']").val(machine_type_id).trigger("change");
            modal.find("select[name='location_id']").val(location_id).trigger("change");
            modal.modal("show");
            window.edit = true;
        });
    });
