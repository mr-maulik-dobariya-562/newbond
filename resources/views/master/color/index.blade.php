                        ...
                        <table class="table table-loader table-vcenter table-hover card-table" id="datatable">
                            <thead>
                                <tr>
                                    ...
                                    <th data-name="font_color">Font Color</th>
...
<form id="modal-form" action="{{ route('master.color.store') }}" enctype="multipart/form-data" method="POST">
    @csrf
    <div class="modal modal-blur fade" id="modal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"> <span class="title">Add</span> Color</h5>
                ...
                <div class="modal-body">
                    <div class="row mb-3 align-items-end">
                        <div class="col">
                            ...
                            <input class="form-control" type="color" name="color" placeholder="Enter Color Name" />
                        </div>
                    </div>
                    <div class="row mb-3 align-items-end">
                        <div class="col">
                            <label class="form-label">Font Color</label>
                            <input class="form-control" type="color" name="font_color" placeholder="Enter Color Name" />
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        var table = window.table(
            "#datatable",
            "{{ route('master.color.getList') }}",
        );
        window.edit = false;

        $(".add-new-btn").click(function() {
            modal.find(".title").text("Add");
            modal.parents("form").attr("action", '{{ route("master.color.store") }}');
            window.edit = false;
        });
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            const fontColor = $(this).data("font_color");
            const edit_url = "{{ route('master.color.update', ':id') }}";
            modal.parents("form").attr("action", edit_url.replace(":id", id));
            modal.find(".title").text("Edit");
            modal.find("input[name='color']").val(color);
            modal.find("input[name='font_color']").val(fontColor);
            modal.modal("show");
            window.edit = true;
        })
    });
