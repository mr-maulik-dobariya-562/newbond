...
@section("content")
    ...
    <form id="modal-form" action="{{ route('master.courier.store') }}" enctype="multipart/form-data" method="POST">
        @csrf
        <div class="modal modal-blur fade" id="modal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"> <span class="title">Add</span> Courier</h5>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = window.table(
                "#datatable",
                "{{ route('master.courier.getList') }}",
            );
            window.edit = false;

            $(".add-new-btn").click(function() {
                modal.find(".title").text("Add");
                modal.parents("form").attr("action", '{{ route("master.courier.store") }}');
                window.edit = false;
            });
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                const edit_url = "{{ route('master.courier.update', ':id') }}";
