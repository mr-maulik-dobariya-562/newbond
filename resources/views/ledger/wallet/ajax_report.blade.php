...
foreach ($data['data'] as $di => $dv) {
    if (!in_array($dv->customer_id, $customer_id)) {
...
foreach ($data['filtered_data'] as $fdi => $fdv) {
    $url = '';
        ?>
        <tr>
            <td><a href="{{ route('wallet-ledger.ledgerReportByCustomer', $fdv['party_id']) }}" target="_blank">
            ...
            <td class="<?= ($closing_amt < 0) ? 'text-danger' : 'text-success' ?>">
                <?= sprintf("%.3f", abs($closing_amt)) ?> <?= ($closing_amt < 0) ? 'DR' : 'CR' ?>
            </td>
        </tr>
        <?php } ?>
    </tbody>
    <tfoot>
        <tr>
            ...
            <th class="<?= ($closing < 0) ? 'text-danger' : 'text-success' ?>"><?= sprintf("%.3f", abs($closing)) ?>
                <?= ($closing < 0) ? 'DR' : 'CR' ?>
            </th>
        </tr>
    </tfoot>
