                            ...
                            <div class="col-md-12">
                                <div class="table-responsive" id="set_table_here">
                                    ...
                                    foreach ($data['opening_data'] as $oIndex => $ov) {
                                        ...
                                        if ($ov->code == 'EST' || ($ov->type == 'payment' && $ov->type == 'DEBIT')) {
                                            $totalOpeningAmount -=  abs($ov['total_net_amt']);
                                        } else if ($ov->code == 'PAY' && $ov->type == 'CREDIT') {
                                            $totalOpeningAmount +=  abs($ov['total_net_amt']);
                                        }
                                    }
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                <tr class="change_color">
                                                    ...
                                                    <?php if ($v->code == 'PAY' && $v->type == 'DEBIT') { ?>
                                                        <?php
                                                    ...
                                                    <?php } else if ($v->code == 'PAY' && $v->type == 'CREDIT') { ?>
                                                        <?php
                                                        $closingAmt += (isset($v->total_net_amt) && !empty($v->total_net_amt)) ? abs($v->total_net_amt) : 0;
    ...
    $(document).ready(function() {
        ...
        function search() {
            ...
            $.ajax({
                ...
                url: "{{ route('ledger.getLedgerCustomerReport') }}",
                method: 'POST',
                showLoader: true,
                data: {
                    fromDate: fromDate,
